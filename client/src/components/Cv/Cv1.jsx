    ...
    const fetchData = async () => {
      try {
        ...
        if (id) {
          ...
          response = await fetch(`http://localhost:8000/api/v1/profile/profile/${id}`, {
            method: "GET",
            credentials: "include",
          });
        } else {
          // If no 'id', fetch the default profile data with credentials
          response = await fetch("http://localhost:8000/api/v1/profile/profile", {
            method: "GET",
            credentials: "include",
          });
        }
        ...
        if (data.personalInfo?.profilePicture) {
          const imageUrl = `http://localhost:8000/${data.personalInfo.profilePicture}`;
        ...
        <div className="flex flex-col md:flex-row">
          <div className="bg-black text-white p-6 md:w-1/2.5">
            ...
            <div className="mb-6 space-y-2">
              ...
              <div className="flex items-center gap-2">
                <strong>
                  {profileData.personalInfo?.gender === "Male" ? <FaMale /> : <FaFemale />}
                </strong>
            ...
            <div className="mb-6">
              ...
              {profileData.language && profileData.language.length > 0 ? (
                profileData.language.map((lang) => (
                  <li key={lang._id} className="flex items-center gap-2 text-sm">
            ...
            <div className="mb-6">
              ...
              {profileData.skills && profileData.skills.length > 0 ? (
                profileData.skills.map((skill) => (
                  <li key={skill._id} className="flex items-center gap-2 text-sm">
          ...
          <div className="p-6 md:w-2/3">
            <div className="mb-6">
              ...
              <p className="text-justify">
                {profileData.summary ? stripHtmlTags(profileData.summary) : "No summary available."}
              </p>
            </div>
            <div className="mb-6">
              ...
              {profileData.education && profileData.education.length > 0 ? (
                profileData.education.map((edu) => (
                  <div className="mb-4" key={edu._id}>
                    ...
                    <p className="text-gray-600">Completed in {edu.completionYear}</p>
                  </div>
                ))
              ) : (
              ...
              {profileData.projects && profileData.projects.length > 0 ? (
                profileData.projects.map((project) => (
                  <div key={project._id} className="mb-4 rounded text-sm">
                    ...
                    <p>
                      <strong>Duration:</strong> {project.startMonth} {project.startYear} -{" "}
                      {project.isOngoing ? "Ongoing" : `${project.endMonth} ${project.endYear}`}
                    </p>
                    <p>
