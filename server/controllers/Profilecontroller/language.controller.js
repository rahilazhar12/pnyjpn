...
// Controller to update languages for a user
// exports.updateLanguages = async (req, res) => {
//   try {
//     const userId = req.user.id; // Get user ID from authenticated user
//     const { country, proficiency } = req.body; // Single language data (country and proficiency)

//     // Validation to ensure country and proficiency are provided
//     if (!country || !proficiency) {
//       return res.status(400).json({ message: "Country and proficiency are required." });
//     }

//     // Find the user's current language entries
//     let userLanguages = await Language.findOne({ userId });

//     if (userLanguages) {
//       // Find the language index if it exists
//       const languageIndex = userLanguages.languages.findIndex(
//         (lang) => lang.country === country
//       );

//       if (languageIndex !== -1) {
//         // If language exists, update the proficiency
//         userLanguages.languages[languageIndex].proficiency = proficiency;
//       } else {
//         // If the language doesn't exist, add it
//         userLanguages.languages.push({ country, proficiency });
//       }

//       // Save the updated languages
//       await userLanguages.save();
//       return res.status(200).json(userLanguages.languages); // Return the updated languages
//     } else {
//       // If no language entry exists, create a new one
//       const newLanguages = [{ country, proficiency }];
//       const newLanguageEntry = new Language({
//         userId,
//         languages: newLanguages,
//       });

//       await newLanguageEntry.save();
//       return res.status(201).json(newLanguageEntry.languages); // Return the newly created languages
//     }
//   } catch (error) {
