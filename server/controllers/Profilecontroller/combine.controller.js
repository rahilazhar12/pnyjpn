...
exports.checkUserIdExistence = async (req, res) => {
  ...
  try {
    ...
    if (experience && Array.isArray(experience.experienceRecords) && experience.experienceRecords.length > 0) {
      foundSchemas.push("Experience");
    }

    // Check in Education schema
    const education = await Education.findOne({ userId });
    if (education && Array.isArray(education.educationRecords) && education.educationRecords.length > 0) {
      foundSchemas.push("Education");
    }

    const language = await Language.findOne({ userId });
    if (language && Array.isArray(language.languages) && language.languages.length > 0) {
      foundSchemas.push("Language");
    }

    const project = await Project.findOne({ userId });
    if (project && Array.isArray(project.projects) && project.projects.length > 0) {
